...
def start_preview(path, fov: float = 60.0, speed_multiplier: float = 1.0):
    ...
    op_id = "lfs_plugins.360_Spiral.operators.path_preview.PATHPREVIEW_OT_preview"
...
def _set_viewport_camera(eye, target, up):
    ...
    if not _api_warning_shown:
        lf.log.warning(
            "360_Spiral: Live preview requires a camera control API that LFS doesn't currently provide. "
